...
#Create an empty dictionary that will store years, its total carbon emissions, and a counter
...
#Divide total carbon emissions per year by the counter
   value = sorted_dict[key][0] / sorted_dict[key][1]
...
for index, row in fastfashion_df.iterrows():
  if row['Country'] in total_countries:
      ...
      count_countries[row['Country']] = 1
for key,value in total_countries.items():
 #Divide total annual waste by counter to find average 
   waste = total_countries[key] / count_countries[key]
...
#Divide total wage by counter to find average 
   wage = total_brands[key] / count_brands[key]
